...
将用户三等分，下发不同的价格。实际上就是：服务端根据用户ID（32位）的首字母将用户三等分，每一份下发不同的json，

### 需求分析

1. 三等分的方法很多，假设是mod 3
2. 首字母是 0,3,6,9,c,e 下载json1
   首字母是 1,4,7,a,d,f 下载json2
   首字母是 2,5,8,b,d 下载json3
3. 寻找 json1,json2,json3 中不同的关键字

根据以上任务内容，需求拆分如下：

1. 随机生成32位用户ID
2. 用户ID和基本的URL去拼接请求获得返回值，拿出返回值中的唯一项
